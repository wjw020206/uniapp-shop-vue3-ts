  ...
  methods: {
    ...
    async open() {
      ...
      if (that.customAction && typeof that.customAction === 'function') {
        ...
        typeof that.localdata !== 'undefined' &&
        that.localdata !== null
      ) {
    ...
    checkSelectShop() {
      ...
      if (that.selectArr.every((item) => item != '')) {
        that.selectShop =
          that.shopItemInfo[that.getArrayToSting(that.selectArr)]
    ...
    checkInpath(clickIndex) {
      ...
      for (let i = 0, len = specList.length; i < len; i++) {
        ...
        for (let j = 0; j < len2; j++) {
          ...
          if (
            that.shopItemInfo.hasOwnProperty(
