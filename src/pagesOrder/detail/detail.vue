  ...
  >
    <template v-if="order">
      ...
      <view class="shipment">
        ...
        <view class="locate">
          <view class="user"
            >{{ order.receiverContact }} {{ order.receiverMobile }}</view
          >
...
const onOrderPay = async () => {
  if (import.meta.env.DEV) {
    ...
    // #ifdef MP-WEIXIN
    // 正式环境微信支付
    const res = await getPayWxPayMiniPayAPI(query.id)
    // 调用微信支付接口
...
const onOrderCancel = async () => {
  const res = await getMemberOrderCancelByIdAPI(query.id, reason.value)
  // 更新订单信息
  order.value = res.result
  // 关闭弹窗
  popup.value?.close?.()
