  ...
  <view class="viewport">
    ...
    <view class="tabs">
      <text
        ...
        :key="item.title"
        @tap="changeActiveIndex(item, index)"
      >
        {{ item.title }}
      </text>
      <!-- 游标 -->
    ...
    >
      ...
      <swiper-item v-for="item in orderTabs" :key="item.title">
        <!-- 订单列表 -->
        <!-- 只有 isRender 为 true 时才渲染 OrderList，避免一次性渲染所有 Tab 内容，发送过多请求 -->
...
const activeIndex = ref(
  orderTabs.value.findIndex((item) => item.orderState === Number(query.type)),
)
/** 默认渲染容器 */
orderTabs.value[activeIndex.value]!.isRender = true

/** 改变选中 tab 下标 */
const changeActiveIndex = (item: OrderTab, index: number) => {
